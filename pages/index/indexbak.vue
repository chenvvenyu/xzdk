<template>
	<view class="container-warp">
		<view class="top-left" @tap="btnMime">
			<image src="../../static/images/dt_false_dl.png"></image>
		</view>
		<view class="top-right"  @tap="btnOrder">
			<image src="../../static/images/dt_false_zc_03.png"></image>
		</view>
		
		<swiper class="banner-index" indicator-dots="1" autoplay="1" interval="3000" duration="2000">
				...
				<view class="con-address">
					<view class="address-item">
						<view class="uni-flex uni-row">
							...
							<view class="btn">
								<view class="box" @tap="btnAddress('mailing',1)"><uni-icons type="location" size="30"></uni-icons></view>
							</view>
						</view>
					</view>
					<view class="address-item">
						<view class="uni-flex uni-row">
							...
							<view class="btn">
								<view class="box" @tap="btnAddress('receipt',1)"><uni-icons type="location" size="30"></uni-icons></view>
							</view>
						</view>
					</view>
				</view>
		...
		<view v-if="!IsShowOutlet" class="agreement-box">
			<label class="radio" style="margin-right: 30rpx;" @tap="agreementChange">
				...
				我已阅读并同意<text @click.stop="GoWebview('userAgreement')">《用户协议》</text>和<text @click.stop="GoWebview('pivacyPolicy')">《隐私政策》</text>
			</label>
		...
		data() {
			return {
				...
				PostData: {
					...
					"mailingPhone": "", //寄件人电话
					"mailingAddress": "从哪儿寄出?", //寄件地址
					"mailingLng": "",//寄件地址经度
					"mailingLat": "",//寄件地址纬度
					"mailingProvince": "",//寄件地址省
					"mailingCity": "",//寄件地址市
					"mailingDistrict": "",//寄件地址县/区
					"mailingRegionID": 0, //寄件区域ID
					"mailingRegion": "", //寄件人区域名称
					"receiptPersonName": "点击填写收件人信息", //收件人姓名
					"receiptPhone": "", //收件人电话
					"receiptAddress": "要寄到哪儿?", //收件人地址
					"receiptLng": "",//收件地址经度
					"receiptLat": "",//收件地址纬度
					"receiptProvince": "",//收件地址省
					"receiptCity": "",//收件地址市
					"receiptDistrict": "",//收件地址县/区
					"receiptRegionID": 0, //收件人区域ID
					"receiptRegion": "", //收件人区域名称
					"remark": "", //备注
				...
				GoodsList: ['文件', '食品', '数码', '药品', '生鲜', '服装', '珠宝', '其它'],
				GoodsIndex: -1,
				banerImg:[],
				polygons:[],
				AddressDesc:'',
	...
	computed:{
		...
		methods:{
				...
				_timeInterval = setInterval(function(){
					if(IntervalTimes>=10) {
						BMap = new bmap.BMapWX({
						    ak: 'k3IbECkECFFWhh3n3B6lu9Ypx5NkGkso' 
						}); 
						uni.getLocation({
							type:'gcj02',
							success(res) {
								...
								BMap.regeocoding({ 
								    ...
								    success: function(data) { 
										...
										if(_obj && _obj.length>0){
											 ...
											 _self.AddressDesc = _obj[0].desc;
											 _self.AddressTitle = _obj[0].address;
											 _self.PostData.mailingAddress = (_self.AddressDesc && _self.AddressDesc.length>0) ? (_obj[0].address + ',' +_obj[0].desc) : _obj[0].address
											let polygons=[]
						...
						if(_self.userInfo.accessToken){
							_self.Get('/api/User/GetCommonlyUsedContactPageList?Type=2&PageSize=1&Range=1',{},_self.userInfo.accessToken,function(dataObj){
								if(dataObj.Status && dataObj.Data && dataObj.Data.rows.length>0) 
								{
									...
									_self.PostData.mailingAddress = _address+','+_desc;
									...
									BMap = new bmap.BMapWX({
									    ak: 'E63DhGMaUEOyScGXLQX9ipxlO9HBUj2j' 
									}); 
									uni.getLocation({
										type:'gcj02',
										success(res) {
			...
			btnSubmit:function(){
				...
				if (VerifyHelper.IsNull(_self.PostData.mailingAddress)) {
					_self.showMsg('请填写寄件地址！');
					return;
				}
				...
				if (VerifyHelper.IsNull(_self.PostData.receiptAddress)) {
					_self.showMsg('请填写收件地址！');
					return;
				}
				...
				if (!_self.Agreement) {
					_self.showMsg('请阅读并同意《用户协议》和《隐私政策》！');
					return;
				}
			...
			btnAddress1(type,curr){
				...
				if(type=='receipt'){
					...
					_item = {
						...
						Address:_self.reAddressTitle,
						Desc:_self.reAddressDesc,
					...
					_item = {
						...
						Address:_self.AddressTitle,
						Desc:_self.AddressDesc,
				...
				uni.navigateTo({
					url:'address?address='+JSON.stringify(_item)+'&type='+type
				})
			},
					...
					if(res.Status){
						if(res.Data && res.Data.length>0){
							...
							for (let i=0;i<res.Data.length;i++) {
								...
								let _range=JSON.parse(res.Data[i].Range)
								if(_range && _range.length>0){
									for(let j=0;j<_range.length;j++){
										let item={
											latitude:_range[j].lat,
			...
			PageReload: function(data, type = 'mailing',title='',desc='') {
				...
				if (data != null) {
					if (type == 'receipt') {
						_self.reAddressDesc = title
						_self.reAddressTitle=desc
			...
			btnLoad: function() {
				...
				if (!VerifyHelper.IsInt(_self.PostData.weight) || Number(_self.PostData.weight) < 3 || Number(_self.PostData.weight) > 300) {
					_self.showMsg('请输入物品重量范围3-300！');
					return;
				}
				...
				uni.navigateTo({
					url: '/pages/index/orderConfirm?data=' + JSON.stringify(_self.PostData)
				})
			},
			getData:function(){
				let t = this;
...
<style lang="scss">
	...
	.address-item{
		...
		.tag{font-size: 28upx;font-weight: bold;padding-right: 30upx;width: 10%;box-sizing: border-box;}
		.content 
		{
		...
		.btn {
			...
			.box{padding-left: 30upx;width: 20%;box-sizing: border-box;}
		}
	}
	.jj_title {
		font-size: 16px;
